package com.example.delivery.Activities.profiles.Delivery.home.fragments
...
import com.example.delivery.Activities.profiles.Delivery.adapters.DeliveryTabsPagerAdapter
...
class DeliveryOrdersFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val adapter = DeliveryTabsPagerAdapter(childFragmentManager, lifecycle, numberOfTabs)
        viewpager?.adapter = adapter
        viewpager?.isUserInputEnabled = true

        TabLayoutMediator(tabLayout!!, viewpager!!) { tab, position ->
            when (position) {
                0 -> tab.text = "DESPACHADO"
                1 -> tab.text = "EN CAMINO"
                2 -> tab.text = "ENTREGADO"
            }
        }.attach()
